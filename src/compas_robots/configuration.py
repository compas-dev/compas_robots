...
if TYPE_CHECKING:
    ...
    from compas_robots.model import JointType
...
class Configuration(Data):
    ...
    joint_values
        ...
        type.
    joint_types
        Joint types, e.g. a list of `JointType.REVOLUTE` for
        revolute joints.
    joint_names
        List of joint names.
    ...
    joint_values : list[float]
        Joint values expressed in radians or meters, depending on the respective
        type.
    joint_types : list[compas_robots.model.JointType]
        Joint types, e.g. a list of `JointType.REVOLUTE` for
        revolute joints.
    ...
    >>> config = Configuration.from_prismatic_and_revolute_values([8.312], [math.pi / 2, 0.0, 0.0, 0.0, 2 * math.pi, 0.8])
    >>> str(config)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if len(joint_values) != len(joint_types):
            raise ValueError("{} joint_values must have {} joint_types, but {} given.".format(len(joint_values), len(joint_values), len(joint_types)))

        if joint_names and len(joint_names) != len(joint_values):
            raise ValueError("{} joint_values must have either 0 or {} names, but {} given".format(len(joint_values), len(joint_values), len(joint_names)))
    ...
    @joint_values.setter
    def joint_values(self, values: list[float]) -> None:
    ...
    @joint_types.setter
    def joint_types(self, joint_types: list[JointType]) -> None:
    ...
    @joint_names.setter
    def joint_names(self, names: list[str]) -> None:
    ...
    def __str__(self):
        ...
        types_tuple = tuple(int(t) for t in self.joint_types)

        if len(self.joint_names):
            return "Configuration(({}), {}, {})".format(values_string, types_tuple, tuple(self.joint_names))
        else:
            return "Configuration(({}), {})".format(values_string, types_tuple)

    def __repr__(self):
        """Printable representation of [Configuration][]."""
        return self.__str__()
    ...
    @classmethod
    def from_revolute_values(cls, values: list[float], joint_names: Optional[list[str]] = None) -> Configuration:
        ...
        -------
        Configuration
            A newly created instance of configuration.
        """
        values = list(values)
    ...
    @property
    def prismatic_values(self) -> list[float]:
        """Prismatic joint values in meters.
    ...
    @property
    def revolute_values(self) -> list[float]:
        """Revolute joint values in radians."""
    ...
    def scaled(self, scale_factor: float) -> Configuration:
    ...
    def iter_differences(self, other: Configuration) -> Iterator[float]:
    ...
    def max_difference(self, other: Configuration) -> float:
    ...
    def close_to(self, other: Configuration, tol: float = 1e-3) -> bool:
    ...
    @property
    def type_dict(self) -> dict[str, JointType]:
